...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] == 0) {
        ...
        $gambar_extension = strtolower(pathinfo($gambar_filename, PATHINFO_EXTENSION)); // Mendapatkan ekstensi file

        // Cek tipe file
        if (in_array($gambar_extension, $allowed_types)) {
            // Membuat nama file unik
            $gambar_new_filename = uniqid() . "" . preg_replace("/[^A-Za-z0-9\-\.]/", '_', $gambar_filename);
            $gambar_target = $target_dir . $gambar_new_filename;

            if (move_uploaded_file($_FILES['gambar']['tmp_name'], $gambar_target)) { //memindahkan file ke direktori
    ...
    if (isset($_FILES['gambar_sumber']) && $_FILES['gambar_sumber']['error'] == 0) {
        ...
        if (in_array($gambar_sumber_extension, $allowed_types)) {
            // Membuat nama file unik
            $gambar_sumber_new_filename = uniqid() . "" . preg_replace("/[^A-Za-z0-9\-\.]/", '_', $gambar_sumber_filename);
    ...
    // Jika tidak ada error, lanjutkan memasukkan data ke database
    if (empty($error)) {
        // Cek apakah judul sudah ada
        $stmt_check = $conn->prepare("SELECT COUNT(*) FROM artikel WHERE judul = ?");
        if ($stmt_check === false) {
            // Log error dan set error message
            error_log("Prepare failed for SELECT COUNT(*): " . $conn->error); // Mencatat pesan kesalahan ke log
            $_SESSION['error_message'] = "Terjadi kesalahan pada database. Silakan coba lagi."; // Mengatur pesan kesalahan di session
            header("Location: tambahArtikel.php");
            exit();
        }
        $stmt_check->bind_param("s", $judul); // Mengikat parameter judul ke statement
        $stmt_check->execute(); 
        $stmt_check->bind_result($count); // Mengikat hasil ke variabel cunt
        $stmt_check->fetch(); //mengambil
        $stmt_check->close();
        ...
        if ($count > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO artikel (judul, tipe, jenis, tgl_publish, sumber, konten, gambar_artikel, gambar_sumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt === false) {
                // Log error dan set error message
                error_log("Prepare failed for INSERT: " . $conn->error);
                $_SESSION['error_message'] = "Terjadi kesalahan pada database saat menyimpan artikel. Silakan coba lagi.";
                header("Location: tambahArtikel.php");
                exit();
            }

            $stmt->bind_param("ssssssss", $judul, $tipe, $jenis, $tanggal, $sumber, $konten, $gambar, $gambar_sumber); // Mengikat parameter ke statement

            if ($stmt->execute()) {
                // Log sukses
                error_log("Artikel ditambahkan oleh user ID: " . $_SESSION['id_admin'] . " dengan judul: " . $judul);
                // Set pesan sukses di session
                $_SESSION['success_message'] = "Artikel berhasil ditambahkan.";
                // Redirect ke tambahArtikel.php untuk mencegah form resubmission
                header("Location: tambahArtikel.php");
                exit();
            } else {
                // Tangani error duplicate entry jika terjadi race condition
                if ($stmt->errno === 1062) { // 1062 adalah kode error untuk duplicate entry
                    $error .= "Judul artikel sudah ada. Silakan gunakan judul lain.<br>";
                    // Log error
                    error_log("Duplicate entry untuk judul: " . $judul . " oleh user ID: " . $_SESSION['id_admin']);
                } else {
                    $error .= "Terjadi kesalahan saat menambahkan artikel: " . htmlspecialchars($stmt->error) . "<br>";
                    // Log error
                    error_log("Error saat menambahkan artikel: " . $stmt->error);
                }
            }
...
<head>
    ...
    <title>Tambah Artikel - RefreshOil</title>
...
<body class="bg-gray-100">
    ...
    <div class="md:ml-64 p-6 mt-4"> <!-- Adjusted top margin to account for fixed header -->
        ...
        <!-- Kartu Form Tambah Artikel -->
        <div class="form-card">
            <form action="tambahArtikel.php" method="POST" enctype="multipart/form-data" autocomplete="off">
                <!-- Input Judul Artikel -->
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="image-preview">
                            <!-- Preview akan ditampilkan oleh JavaScript -->
                        </div>
    ...
    <script>
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    if (img) {
                        ...
                        newImg.classList.add('w-32', 'h-32', 'object-cover', 'rounded', 'border', 'border-gray-300');
        ...
        // Event listeners untuk input gambar
        const gambarInput = document.getElementById('gambar');
        const gambarSumberInput = document.getElementById('gambar_sumber');
        ...
        if (gambarSumberInput) {
            gambarSumberInput.addEventListener('change', function() {
                const previewContainer = this.parentElement.querySelector('.image-preview');
                previewImage(this, previewContainer);
            });
        }
