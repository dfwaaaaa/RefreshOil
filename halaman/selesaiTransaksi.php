...
$iframe = isset($_GET['iframe']) && $_GET['iframe'] == '1'; // Deteksi parameter 'iframe' di URL
...
function format_tanggal_indonesia($tanggal) { 
    $bulan = [ // Daftar bulan dalam bahasa Indonesia
        1 => 'Januari',
        'Februari',
        'Maret',
        'April',
    ...
    $timestamp = strtotime($tanggal); // Konversi string tanggal ke timestamp
    if (!$timestamp) { // Jika konversi gagal
...
// Override semua status menjadi 'selesai'
$status_pembayaran = 'paid'; 
$status_petugas = 'selesai'; 
...
$status_orderInfo = [ // Informasi status order
    'bg_class' => 'bg-green-500', 
    'label' => 'Selesai',
...
try {
    ...
    $stmt = $pdo->prepare("SELECT * FROM users WHERE id = :id LIMIT 1"); // Query untuk mengambil data user
    $stmt->bindParam(':id', $_SESSION['user_id'], PDO::PARAM_INT); // Bind parameter 'id' dengan 'user_id' dari sesi
    $stmt->execute(); // Eksekusi query
    $admin = $stmt->fetch(PDO::FETCH_ASSOC); // Ambil hasil sebagai array asosiatif
    ...
    $transaksiStmt = $pdo->prepare("SELECT * FROM penjemputan WHERE id_penjemputan = :id LIMIT 1"); // Query untuk mengambil detail transaksi
    $transaksiStmt->bindParam(':id', $transaksiId, PDO::PARAM_STR); // Bind parameter 'id' dengan ID transaksi
    $transaksiStmt->execute(); // Eksekusi query
    $penjemputan = $transaksiStmt->fetch(PDO::FETCH_ASSOC); // Ambil hasil sebagai array asosiatif
    ...
    if (!$penjemputan) { 
        ...
        $userStmt = $pdo->prepare("SELECT nama, category, phone, profil_img FROM users WHERE id = :id LIMIT 1"); // Query untuk mengambil data pemesan
        $userStmt->bindParam(':id', $userId, PDO::PARAM_INT); // Bind parameter 'id' dengan ID pengguna
        $userStmt->execute(); // Eksekusi query
        $pemesan = $userStmt->fetch(PDO::FETCH_ASSOC); // Ambil hasil sebagai array asosiatif
          ...
          <div class="mt-2">
              <div class="flex justify-center gap-8 flex-col lg:flex-row">
                  ...
                  <div class="flex flex-col w-full lg:w-3/4 bg-gray-100 p-6 rounded-lg">
                      ...
                      <p class="text-sm text-green-600 font-bold">Selesai</a></p>
                      </div>
                  </div>
